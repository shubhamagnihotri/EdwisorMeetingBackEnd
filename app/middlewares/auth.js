  ...
  if (req.params.authToken || req.query.authToken || req.body.authToken || req.header('authToken')) {
    Auth.findOne({authToken: req.header('authToken') || req.params.authToken || req.body.authToken || req.query.authToken}, (err, authDetails) => {
      if (err) {
        ...
        let authResponse ={p_response_code:'1001'} //for failed to authrized
        let apiResponse = responseLib.generate(true, 'Failed To Authorized', 500, authResponse)
        res.status(500)
        res.send(apiResponse)
      } else if (check.isEmpty(authDetails)) {
        logger.error('No AuthorizationKey Is Present', 'AuthorizationMiddleware', 10)
        let authResponse ={p_response_code:'1001'} //for failed to authrized
        let apiResponse = responseLib.generate(true, 'Invalid Or Expired AuthorizationKey', 404, authResponse)
        res.status(404)
        res.send(apiResponse)
      } else {
        token.verifyToken(authDetails.authToken,authDetails.tokenSecret,(err,decoded)=>{
            ...
            if(err){
                ...
                req.user = {userId: decoded.data.userId,userType:decoded.data.role}
                req.user.name = decoded.data.role == 'admin' ? decoded.data.firstName+'-admin' :decoded.data.firstName+'-user';
                next()
            }
